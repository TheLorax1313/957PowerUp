...
public class MainControlSystem extends TimedRobot {
	...
	VisionSubsystem.Position trackingPosition = VisionSubsystem.Position.CENTER_X;
	...
	public void autonomousPeriodic() {
		...
		if(m_autoMode == 6) {
			...
			if(m_scaleLocation.equals("L")) {
				...
				case 5:
					...
					if(m_ahrs.getYaw() < 120) {
						speed = 0.65;
					}else {
						speed = 0.6;
					}
					
					if(m_ahrs.getYaw() > 130) {
						m_drive.tankDrive(0, 0);
						m_autoStep = 6;
					}
					m_drive.tankDrive(speed,-speed);
		...
		if(m_autoMode == 8) {
			...
			case 2:
				...
				//System.out.println("switch location: " +m_switchLocation);
				if(m_switchLocation.equals("L")) {
					if(m_ahrs.getYaw() < -35) {
						speed = 0.55;
					}else {
			...
			// Turn until the edge of the cube pile is in front of us
			case 8:
				System.out.println("STEP 8");
				headingToHold = m_ahrs.getYaw();
				
				// Turn Left
				if(m_switchLocation.equals("R")) {
					m_drive.tankDrive(-0.6, 0.6);
					if(m_vision.getPosition(VisionSubsystem.Position.LEFT) > -5) {
						m_drive.tankDrive(0, 0);
						m_autoStep = 9;
						trackingPosition = VisionSubsystem.Position.LEFT;
						m_encoders.reset();
					}
				}
				...
				if(m_switchLocation.equals("L")) {
					m_drive.tankDrive(0.6, -0.6);
					if(m_vision.getPosition(VisionSubsystem.Position.RIGHT) > -5 && m_vision.getPosition(VisionSubsystem.Position.RIGHT)!=666) {
						m_drive.tankDrive(0, 0);
						m_autoStep = 9;
						trackingPosition = VisionSubsystem.Position.RIGHT;
						m_encoders.reset();
					}
				}
			...
			case 9:
				m_arm.grab();
				//driveStraight(0.5, headingToHold);
				autoDrive(0.5,2, m_vision.getPosition(trackingPosition));
				if(!m_beam2.get()) {
					m_arm.stop();
					m_drive.tankDrive(0, 0);
					m_autoStep = 10;
	...
	public void robotPeriodic(){
		SmartDashboard.putNumber("position", m_vision.getPosition(VisionSubsystem.Position.LEFT));
		SmartDashboard.putBoolean("true", true);
		SmartDashboard.putNumber("testNav", m_ahrs.getYaw());
	}
